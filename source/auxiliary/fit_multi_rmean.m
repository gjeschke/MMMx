function fom = fit_multi_rmean(v,fit,opt)
% Computes the distance distributions for an ensemble and root mean square
% deviation of the mean distances with respect to restraint distributions
%
...
for k = 1:length(fit)
    ...
    diff = sum(sim.*rax)-sum(restraint.*rax);
    fom = fom + diff^2;
end

fom = sqrt(fom/length(fit));
...
if isfield(opt,'interactive') && opt.interactive
    if mod(call_count,opt.update_nr) == 0
        ...
        title(sprintf('Mean distance RMSD(%i): %5.3f A with %i conformers',call_count/1000,fom,length(coeff)));
        drawnow
    end
