function [acodes,transmat] = get_RNA_initial_anchor(logfid,fragments,anchor,base)
% function [acodes,transmat] = get_RNA_initial_anchor(logfid,fragments,anchor,base)
% 
...
%               initial anchor nucleotide, O3' of previous and next nt can
%               be included
...
acodes = [];
while isempty(acodes) && anchor_acc <= 3 && O3p_acc <= 3 
    ...
    if isnan(min(rmsd_vec)) || isempty(acodes)
        fprintf(logfid,'Minimal anchor rmsd: %5.2f Å\n',min(rmsd_vec));
    end
    na = length(acodes);
    if na < 5
        fprintf(logfid,'Warning: Only %i fragments fit initial anchor coordinates with requested accuracy of %4.1f Å\n',na,anchor_acc);
    end
    if na < 1
        anchor_acc = anchor_acc + 0.5;
        fprintf(logfid,'Warning: Anchor accuracy increased to %4.1f Å\n',anchor_acc);
    end
    if m >= 6
        ...
        na2 = length(acodes);
        if na2 < 5
            fprintf(logfid,'Warning: Only %i fragments fit O3'' anchor coordinate with requested accuracy of %4.1f Å\n',na2,O3p_acc);
        end
        if na2 < 1
            O3p_acc = O3p_acc + 0.5;
            fprintf(logfid,'Warning: Accuracy of O3'' anchor coordinate incresaed to %4.1f Å\n',O3p_acc);
        end
    end
end
