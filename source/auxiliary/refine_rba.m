function [atransmat,errvec,model_prob,xld,cost,costs,aux_fulfill] = refine_rba(rb,atransmat,points,...
...
sl_clash_fail = 10; % value of the stem loop clash cost function were clashes are considered as too strong 
...
% v1 = fminsearch(@rba_cost_fct,v0,options,atransmat,rb,points,naux,auxiliary,ncore,core,links,clash_threshold,heavy_coor,stemlibs,combination,sl_atoms);

% lb = v0 - 0.6*ones(size(v0));
% ub = v0 + 0.6*ones(size(v0));

if ~isempty(combination) && preliminary_verbose
    [~,costs] = rba_cost_fct(v0,atransmat,rb,points,naux,auxiliary,ncore,core,links,clash_threshold,heavy_coor,stemlibs,combination,sl_atoms,pthr,clash_fail,false,v0);
...
tstart = tic;
if ~isempty(combination)
    [v1,cost] = fminsearch(@rba_cost_fct,v0,options,atransmat,rb,points,naux,auxiliary,ncore,core,links,clash_threshold,heavy_coor,stemlibs,combination,sl_atoms,pthr,clash_fail,do_echo,v0);
else
    [v1,cost] = fminsearch(@rba_cost_fct,v0,options,atransmat,rb,points,naux,auxiliary,ncore,core,links,clash_threshold,heavy_coor,stemlibs,combination,sl_atoms,pthr,clash_fail,do_echo,v0);
end
telapsed = toc(tstart);
...
[~,costs] = rba_cost_fct(v1,atransmat,rb,points,naux,auxiliary,ncore,core,links,clash_threshold,heavy_coor,stemlibs,combination,sl_atoms,pthr,clash_fail,false,v0);
if ~isempty(combination)
...
for kr1 = 1:length(rb)-1
    ...
    for kr2 = kr1+1:length(rb)
        ...
        rcost = clash_cost(hc1,hc2,clash_threshold);
        if rcost > max_cost
            max_cost = rcost;
        end
        if rcost > clash_fail
...
%         baspoi4 = 4*(kr1-1);
%         transmat = atransmat(baspoi4+1:baspoi4+4,:);
%         hc1 = affine_coor_set(heavy_coor{kr1},transmat);
%         for kr2 = kr1+1:length(rb)
%             baspoi4b = 4*(kr2-1);
%             transmatb = atransmat(baspoi4b+1:baspoi4b+4,:);
%             hc2 = affine_coor_set(heavy_coor{kr2},transmatb);
%             cost_clash = cost_clash + clash_cost(hc1,hc2,clash_threshold);
%         end
%     end
    for kr = 1:length(rb)
        baspoi4b = 4*(kr-1);
