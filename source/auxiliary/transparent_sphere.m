function transparent_sphere(midpoint,radius,color,opaqueness)

if ~exist('opaqueness','var') || isempty(opaqueness) % default is an opaque sphere
    opaqueness = 1;
end

if ~exist('color','var') || isempty(color) % default color is blue
    color = [0,0,1];
end

if ~exist('radius','var') || isempty(radius) % default radius is 1
    radius = 1;
end

if ~exist('midpoint','var') || isempty(midpoint)
    midpoint = [0,0,0];
end

[x,y,z,t] = point2trisphere(midpoint,radius,2);
obj = trisurf(t,x,y,z);
set(obj, 'FaceColor', color, 'EdgeColor', 'none','FaceLighting','gouraud','Clipping','off');
set(obj, 'CDataMapping','direct','AlphaDataMapping','none','FaceAlpha',opaqueness);
