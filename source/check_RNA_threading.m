function threaded = check_RNA_threading(coor_prot,coor_RNA,atomtags)
%
% CHECK_RNA_THREADING   Checks if an RNA is threaded through a hole in the
%                       protein
%
%   threaded = CHECK_RNA_THREADING(coor_prot,coor_RNA,atomtags)
%   Returns a Boolean flag that indicates whether an RNA is close to
%   protein atoms in all directions, a single nucleotide is always
...
% atomtags     atom tags corresponding to RNA coordinates, only C4' and C2'
%               coordinates are used
%
% OUTPUT
% threaded     true, if the RNA is too close to protein atoms in all
%              directions, false if not; empty if the number of C4' atoms
%              does not match the number of C2'atoms
%

% This file is a part of MMMx. License is MIT (see LICENSE.md). 
% Copyright(c) 2021: Gunnar Jeschke

threshold = 3.6; % threshold for close approach, optimized on SRSF1
test_radius1 = 2.0; % smaller test radius, optimized on SRSF1
test_radius2 = 5.0; % larger test radius, optimized on SRSF1
...
n2 = 0;
for k = 1:n0
    if strcmp(atomtags{k},'C4''')
        n4 = n4 + 1;
        C4p_coor(n4,:) = coor_RNA(k,:);
    end
    if strcmp(atomtags{k},'C2''')
        n2 = n2 + 1;
        C2p_coor(n2,:) = coor_RNA(k,:);
    end
end
if n2 ~= n4 
    threaded = [];
    ...
    C4p_coor = C4p_coor(1:n4,:);
    C2p_coor = C2p_coor(1:n2,:);
end
if n2 < 2
    threaded = false;
    return
...
spr = 6;
[backbone,rung,normal] = mk_ribbon(spr,C4p_coor,C2p_coor);
...
for k = 1:length(backbone)
    ...
    for a = 1:length(angi)
        ...
        diff = sqrt(sum((coor_prot - repmat(point,np,1)).^2,2));
...
approaches = max([approaches1;approaches2]);
% Diagnostic plotting
