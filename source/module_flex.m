...
initial_ensemble = '';
added_conformers = '';
OpenMM_script = ''; % empty file name for OpenMM minimizer script 
pH = 7.0; % default pH value for OpenMM minimizer script
...
for d = 1:length(control.directives)
    switch lower(control.directives(d).name)
        ...
        case 'depth'
            ...
            fprintf(logfid,'\n\n');
        case 'optimizer'
            OpenMM_script = control.directives(d).options{1};
            if length(control.directives(d).options) >= 2
                pH = str2double(control.directives(d).options{2});
            end
        otherwise
            warnings = warnings + 1;
...
% write minimzer script, if this was requested
if ~isempty(OpenMM_script)
    filelist = dir(sprintf('%s_i*.pdb',savename));
    mk_OpenMM_minimize_script(OpenMM_script,filelist,pH);
end
