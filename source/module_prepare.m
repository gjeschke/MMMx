...
% get_PED       get ensemble from PED
% merge         build a combined structure from parts of other structures
% oligomer      generate an oligomer from (part of) an entity, the n
%               protomers are generated by rotation about the z axis
...
for d = 1:length(control.directives)
    ...
    switch lower(control.directives(d).name)
        ...
        case 'get_ped'
            ...
            if length(args) > 1
                ensemble = args{2};
                entity_descriptor.entity = get_PED(entry,ensemble); % load the entity
            else
                entity_descriptor.entity = get_PED(entry); % load the entity
            end
            entity_descriptor.conformer = conformer;
            entities{entity_poi} = entity_descriptor;
            commands{cmd_poi} = cmd;
        case 'getens'
            ...
            entity_descriptor.entity = get_ensemble(cmd.input); % load the entity
            entity_descriptor.conformer = conformer;
            entities{entity_poi} = entity_descriptor;
            commands{cmd_poi} = cmd;
        case 'getalphafold'
        ...
        case 'oligomer'
            ...
            if length(control.directives(d).options) > 1
                cmd.n = str2double(control.directives(d).options{2});
            else
                cmd.n = 2;
            end
            if length(control.directives(d).options) > 2
                cmd.outname = control.directives(d).options{3};
            else
                cmd.outname = sprintf('%s_%i-mer',cmd.entity,cmd.n);
            end
            if length(control.directives(d).options) > 3
                [~,chain,range] = split_conf_chain_range(control.directives(d).options{4});
                cmd.chain = chain;
                cmd.range = range;
            else
                cmd.chain = '';
...
for c = 1:cmd_poi
    ...
    switch cmd.name
        ...
        case 'oligomer'
            fprintf(logfid,'\nGenerating oligomer from entity %s\n',cmd.entity);
            if strcmp(cmd.entity,'.')
                c_entity = entity;
            else
                c_entity = get_entity(cmd.entity,entities,logfid);
                if isempty(c_entity)
                    warnings = warnings +1;
                    exceptions{warnings} = MException('module_prepare:entity_unknown',...
                        'tried to generate oligomer from entity %s, which is unknown',cmd.entity);
                    record_exception(exceptions{warnings},logfid);
                    return
                end
                entity_name = cmd.entity;
            ...
            conformers = combine(C,cmd.n);
            combis = C^cmd.n;
            if isempty(cmd.chain)
                chains = fieldnames(c_entity);
            ...
            if isempty(cmd.range)
                ...
                cmd.range = [minres,maxres];
            end
            outname = sprintf('%s.ens',cmd.outname);
            ofid = fopen(outname,'wt');
            fprintf(ofid,'%% %s, %s\n',outname,datetime);
            for combi = 1:combis
                for protomer = 1:cmd.n
                    parts(protomer).conformer = conformers(combi,protomer); %#ok<AGROW>
                    parts(protomer).chain = cmd.chain; %#ok<AGROW>
                    parts(protomer).range = cmd.range; %#ok<AGROW>
                end
                [entity,failed] = mk_oligomer(c_entity,parts,cmd.n,logfid);
                fname = sprintf('%s-m-%i.pdb',cmd.outname,combi);
...
function [entity,failed] = mk_oligomer(c_entity,parts,n,logfid)
...
occupancies = ones(100000,1,'uint8');
atnum = 0;
angles = 2*pi*(0:1:n-1)/n;
for p = 1:n
    transmat = affine('rotz',angles(p));
    xyz0 = affine_coor_set(c_entity.xyz,transmat); % transform coordinates for this protomer
    ctag = char(double('A')+p-1); % chain tag for this part in new entity
    entity.(ctag).selected = false;
    entity.(ctag).index = p;
    conf = parts(p).conformer;
    ...
    if isempty(c_entity)
        ...
        fprintf(logfid,'New chain %s derived from [%s]{%i}(%s)%i-%i\n',ctag,c_entity.name,parts(p).conformer,chain,parts(p).range);
    end
    for r = range % loop over all selected residues
        residue = sprintf('R%i',r);
        entity.(ctag).(residue).index = 1 + r - range(1);
