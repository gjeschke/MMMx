function [entity,failures] = pair_list(entity,sites,label,fname,options)
%
...
% sites     structure with testet labelling sites, only field .address is
%           used
...
fid = fopen(fname,'wt');
fprintf(fid,'%% MMMx site pair list for label: %s\n',label);
fprintf(fid,'%% Site1        Site2     rmean (%s)  sigr (%s)\n',char(197),char(197));

S = length(sites);

for s1 = 1:S-1
    c1 = which_conformer(sites(s1).address);
    for s2 = s1+1:S
        c2 = which_conformer(sites(s2).address);
        if c1 == c2 || ~options.coupled
            [r_axis,distribution,entity] = distance_distribution(entity,...
                sites(s1).address,label,sites(s2).address,label,options);
            ...
            site1 = sites(s1).address;
            site2_str = pad(sites(s2).address,20-length(site1),'left');
