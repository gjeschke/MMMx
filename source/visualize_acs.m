function visualize_acs(coor,errors_3D,assignment,weights,figname)

[C,~] = size(coor);
ensembles = max(assignment);
if ensembles > 1
    cmap = jet(ensembles);
else
    cmap = [0,0,0.75];
...
for c = 1:C
    ensemble = assignment(c);
    color = cmap(ensemble,:);
    radius = errors_3D(c);
    midpoint = coor(c,:);
    weight = weights(c)/max(weights);
    conformer_sphere(midpoint,radius,color,weight);
end
xrange = max(coor(:,1)) - min(coor(:,1));
yrange = max(coor(:,2)) - min(coor(:,2));
zrange = max(coor(:,3)) - min(coor(:,3));
axis equal
set(gca,'FontSize',14);
title(sprintf('3D ACS %4.1f%c%4.1f%c%4.1f %c^3',xrange,char(215),yrange,char(215),zrange,char(197))); 
my_axes = gca;
view(my_axes,[0,0,1]);
...
lighting gouraud
lh = camlight;
saveas(h,sprintf('ACS_%s_z.png',figname));
view(my_axes,[0,1,0]);
my_axes.CameraUpVector = [1,0,0];
camlight(lh);
saveas(h,sprintf('ACS_%s_y.png',figname));

function conformer_sphere(coor,grace,col,weight)
[x,y,z,t] = point2trisphere(coor,grace/2,2);
obj = trisurf(t,x,y,z);
set(obj, 'FaceColor', col, 'EdgeColor', 'none','FaceLighting','gouraud','Clipping','off','FaceAlpha',weight);
